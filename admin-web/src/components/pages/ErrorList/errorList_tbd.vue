...
export default {
  ...
  methods: {
    ...
    async submitAddCreditForm() {
      ...
      let checkTurn = document.getElementById("2"); //0 //1
      let checkamount = document.getElementById("amountadd"); // 4 //5
      const res  = await checkmemberService.getCheckByUuid_X(checkCode)
      const x_turnval = res.deposit_latest_one_with_promotion.ref_turn_over.error_list_turnover.turn_over;
      // console.log('1.checkcode')
      // console.log(checkCode) 
      console.log('2.checkTurn')
      console.log(checkTurn.value) 
      console.log('3. checkAmount')
      console.log(checkamount.value)
      console.log('4. x_turnval')
      console.log(x_turnval) // 0
      ...
      // //ยอดที่ต้องทำ เมื่อเติมเครดิต
      // //อัพเดทยอดที่ต้องทำก่อนหน้าและปัจจุบัน (รวม)
      update_turn_amt = Number(checkamount.value)*Number(checkTurn.value);//4*0 = 0 //5*1= 5
      update_final_turn_amt = Number(x_turnval)+Number(update_turn_amt); //0+0 = 0 //0+5 = 5
      console.log('ยอดที่ต้องทำเมื่อเติมเครดิต b4')
      console.log(update_turn_amt)
      console.log('ยอดที่ต้องทำเมื่อเติมเครดิต final')
      console.log(update_final_turn_amt)

      
      // // this.formadd.turn_over.turn_over = addVal_X;




      if (Number(x_turnval)>=Number(checkTurn.value)){
        // console.log('IF')
        new_turnover_val = Number(x_turnval);
        // console.log(new_turnover_val)
      } else if(Number(x_turnval)==0 && Number(checkTurn.value)==0){
        new_turnover_val = 0;
        // console.log('ELIF')
        // console.log(new_turnover_val)
        }
        ...
        let getARR =[]
        getARR.push(update_final_turn_amt);
        getARR.push(this.formadd.annotation);
        console.log(getARR)


        this.formadd.turn_over.turn_over = new_turnover_val;
        this.formadd.annotation = getARR;
        // this.formadd.turn_over.total_amt = update_final_turn_amt;

        console.log(this.formadd.turn_over.turn_over)//1
        console.log(update_final_turn_amt)//1
        console.log(checkCode.value)
        // console.log(this.$store.state.test_amt)//1

        // try{
        //   localStorage.setItem('username', checkCode.value);
        //   localStorage.setItem('new_turn_over',this.formadd.turn_over.turn_over );
        //   localStorage.setItem('new_todo_amt', update_final_turn_amt);
        ...
        // this.formadd.turn_over.todo_amt = update_final_turn_amt;


        // let Arrhere = [];
        // Arrhere.push(new_turnover_val);
        // Arrhere.push(update_final_turn_amt);
        // Arrhere.push(res.deposit_latest_one_with_promotion.ref_turn_over.error_list_turnover.ref_deposit_uuid);
        // console.log(Arrhere)

        // try{
        //   await errorlisttomembercheck.passVal(Arrhere);
        //   setTimeout(function () { 
        //     const GetVerifyResponse = localStorage.getItem('passVal_X');
        //     const GetVerifyResponseData = JSON.parse(GetVerifyResponse);
        //     console.log('480')
        //     console.log(GetVerifyResponseData)
...
// this.formadd.turn_over.turn_over ="0"; //fpr fix

...
exports.callApprovedSmsSCB = async (req,res,next) => {
  try {
    console.log("=callApprovedSmsSCB=",req.body)
    ...
    if (indexOf !== -1) {
      ...
      if (indexOfFirst != -1) {
        ...
        if (memberAccountBank !== null) {
          ...
          const existSms = await models.Sms_Slip_Transaction.findOne({
            where: {
              bank_date: bank_date,
              bank_time: bank_time,
              amount: amount,
          ...
          const smsSlipTransaction = await models.Sms_Slip_Transaction.create({
            uuid: uuidv4(),
...
exports.getSmsListSCB = async (req, res, next) => {
  try {
    ...
    const smsSlipTransaction = await models.Sms_Slip_Transaction.findAll({
      order: [["id", "DESC"]],
      include: [
        {
...
exports.getSmsSCBByUUID = async (req, res, next) => {
  try {
    ...
    const smsSlipTransaction = await models.Sms_Slip_Transaction.findOne({
      where: {
        uuid: uuid,
      },
...
exports.approvedSmsSCB = async (req, res, next) => {
  try {
    ...
    const smsSlipTransaction = await models.Sms_Slip_Transaction.update(
      {
    ...
    const account_transection = await models.Member_Account_Bank_Transaction.create(
      {
        ...
        sms_slip_transaction_uuid: uuid, //
      }
    );
   ...
   ///////////////////////
    res.status(200).json({
      message: "เพิ่มข้อมูลเรียบร้อยแล้ว",
      smsSlipTransaction: smsSlipTransaction,
    ...
    const account_transection = await models.Member_Account_Bank_Transaction.create(
      {
        ...
        username: member.sb_username,
        promotion_uuid: resp.uuid,
        sms_slip_transaction_uuid: sms_slip_transaction_uuid,
      }
    );
...
exports.insertKbankSms = async (req, res, next) => {
  try {
    ...
    if(phoneNumber == "ttbbank"){
      const searchMoney = "มีเงิน";
      const indexOfMoney = spsms[0].indexOf(searchMoney);
      const searchend = "บ.";
      const indexOfend = spsms[0].indexOf(searchend);

      const searchAcct = "โอนข้ามบ/ช";
      const indexOfSearchAcct = spsms[0].indexOf(searchAcct);
      const searchReamain = "เหลือ";
      const indexOfSearchRemain = spsms[0].indexOf(searchReamain);

      const searchTime = "@";
      const indexOfSearchTime = spsms[0].indexOf(searchTime);
      const txtDate = spsms[0].substring(indexOfSearchRemain, spsms[0].length);
      const searchDate = "บ."
      const indexOfSearchDate = txtDate.indexOf(searchDate);
      const indexOfSearchEndDate = txtDate.indexOf(searchTime);
      amount = spsms[0].substring(searchMoney.length, indexOfend)
      bank_acct = spsms[0].substring(indexOfSearchAcct + searchAcct.length, indexOfSearchRemain)
      bank_date = txtDate.substring(indexOfSearchDate + searchDate.length, indexOfSearchEndDate)
      bank_time = spsms[0].substring(indexOfSearchTime + searchTime.length , spsms[0].length)
      ...
      if (result) {
        ...
        if (checkintran) {
          ...
          //checktwo
          const checktwoexistSms = await models.Sms_Slip_Transaction.findOne({
            where: {
              bank_date: bank_date,
              bank_time: bank_time,
              amount: amount,
          ...
          const existSmsde = await models.Sms_Slip_Transaction.findOne({
            where: {
              bank_date: bank_date,
              bank_time: bank_time,
              amount: amount,

    ...
    >
      <div class="col-md-9">
        ...
        >
          ...
          <form class="text-center" id = 'slipsubmitform'>
              <div class="form-group p-5 rounded py-1"
              style="background-color: rgb(27, 27, 27);"
              >
                <label for="exampleFormControlFile1"><i class="fa-sharp fa-solid fa-image"></i> สลิปธนาคาร (ขนาดไม่เกิน 2 MB)</label>
                <input
                  type="file"
                  class="form-control-file "
                  id="exampleFormControlFile1"
                  />
                  <button type="submit" class="btn btn-success mt-4">ยืนยันการแจ้งโอนเงิน</button>
              </div>
            </form>
          </div>
          ...
          <div class="row mt-5">
            <div class="col-12">
              ...
              <p>
                - หลังจากโอนเงินเสร็จ ระบบจะเติมเครดิตเข้าอัตโนมัติ ภายใน 10-30
                วินาที
              </p>
...
export default {
  ...
  mounted() {
    ...
    var from_data_response ='';

    async function confirmToDeposit(event) {
        event.preventDefault();
        var fileSlip = document.getElementById('exampleFormControlFile1').files[0];
        var toBankNum = document.getElementById("tbnum").innerHTML;
        var toBankName = document.getElementById("tbname").innerHTML;
        //กรุณาโอนเงินจากบัญชี
        from_data_response = await withdrawService.getDataUser();
        // console.log(from_data_response)
        var submit_pass_data ={
             "uuid": from_data_response.user.uuid,
             "sb_username": from_data_response.user.sb_username,
             "tel": from_data_response.user.tel,
             "from_bank_account_number":from_data_response.data.bank_number,
             "from_bank_account_name":from_data_response.data.bank_account_name,
             "from_bank_account_namebank":from_data_response.nameBank,
             "file_Slip":fileSlip,
             "to_bank_account_number":toBankNum,
             "to_bank_account_name":toBankName,
            //  "to_bank_account_namebank":toNameBank,
} 
        // From account Name - from_data_response.data.bank_account_name
        // From account Num - from_data_response.data.bank_number
        console.log(submit_pass_data)
        axios.post('http://localhost:5001/api/slip/upload', 
        submit_pass_data)
    ...
    let confirmToattached = document.getElementById("slipsubmitform");
    confirmToattached.addEventListener("submit", (event) => confirmToDeposit(event));
  ...
  methods: {
    ...
    //         var tmp =response.data.data[i]
    //         if(tmp.type_deposit =="0"){
    //             var ans = tmp;
